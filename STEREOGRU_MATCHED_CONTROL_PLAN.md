...
日期：2026-09-10。状态：**v2 的 seed0 C0/C1 均已完成 1000 步，首轮结果为指标取舍，未达到一致提升。** 下一步只补同协议 paired seeds 1/2，不改预算/目标，也不加 GRU。C0、C1 结果分别见第 8、9 节。

**当前执行版本为 v2：30 train / 16 dev。** v1 的阿里云准备阶段确认 Scene02 仅有 15 个满足运动范围与 8 帧起点间隔的窗口，无法满足每训练场景 16 个；当时任何 arm 均未训练，随后显式将 Scene01/02 各改为 15 个，保持场景平衡、dev=16、运动/间隔/预算不变。v2 配额现已通过，seed0 的 C0/C1 均完成。旧配置和失败输出保留，不绕过配额检查。详见第 7–9 节。
...
**本轮 readout 到此收束。** 不再要求用户反复运行同类 zero/flat/reverse 诊断。下面是已实现的执行边界，C0/C1 已得到首个 paired seed 的真实训练结果；重复验证仍使用相同协议。
...
该入口仅为预登记的两个 seed 追加，不改变已完成 seed0 的任何结果或训练代码；真实 seed1/2 尚未执行。
