...
日期：2026-09-10。状态：**v2 的真实 C0 已完成 1000 步，下一步执行同一实验的 C1。** 尚无 C1/GRU 增益结论；不继续堆同类诊断或自动开启 GRU。C0 结果见第 8 节。

**当前执行版本为 v2：30 train / 16 dev。** v1 的阿里云准备阶段确认 Scene02 仅有 15 个满足运动范围与 8 帧起点间隔的窗口，无法满足每训练场景 16 个；当时任何 arm 均未训练，随后显式将 Scene01/02 各改为 15 个，保持场景平衡、dev=16、运动/间隔/预算不变。v2 配额现已通过，C0 已完成。旧配置和失败输出保留，不绕过配额检查。详见第 7–8 节。
...
**本轮 readout 到此收束。** 不再要求用户反复运行同类 zero/flat/reverse 诊断。下面是已实现的执行边界，C0 已得到真实训练结果，C1 尚待同协议运行。
...
真实 v2 配额已通过、C0 已完成；后续使用同一 v2 实验目录运行 C1，不能重新生成清单或初值。
