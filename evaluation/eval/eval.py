...
def get_gt(depth_gt_path, gt_factor, args):
    ...
    elif depth_gt_path.split('.')[-1] == 'dsp5':
        with h5py.File(depth_gt_path, "r") as f:
            disparity = f["disparity"][()]
            disparity = disparity[::2, ::2]
        mask=disparity>0
        depth_gt = np.zeros_like(disparity, dtype=np.float32)
        depth_gt[mask]=1/disparity[mask]
    else:
...
def main():
    ...
    for dataset in args.datasets:
        ...
        if dataset == 'kitti_500':
            ...
            args.json_file = os.path.join(args.benchmark_path,'mvs_synth/mvssynth_video_tae.json')
            args.root_path = os.path.join(args.benchmark_path,'mvs_synth')
