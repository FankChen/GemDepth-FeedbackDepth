...
- 评测：KITTI Eigen（`kitti_video.json`），disparity 空间 lstsq scale/shift 对齐（与 baseline 完全一致）

---

## 一、TL;DR

多尺度 refine head 训练本身正常收敛（loss 56.4→4.85）。但**首次评测 AbsRel 0.243，异常偏高**。定位后确认**不是方法差，是训练时辅助损失把模型输出从「视差空间」拉到了「米制深度空间」，与评测假设错配**。已按"复制一份、不动原件"的方式做了修复副本，正在阿里云重训验证。
...
| **multiscale_fix（本次修复，重训中）** | 待测 | 待测 | 待测 | 视差空间 aux |

诊断：multiscale 输出 `min/median/max = 4.73 / 26.1 / 195.3` → 是**米制深度**（视差应为 0.01~0.3）。

---

## 三、问题分析（根因）

模型整体在**视差空间**（disparity = 1/depth）：主损失 SSI、评测对齐都按视差。但 multiscale 的**辅助损失把输出拉向了米制深度**，两者互为倒数，仿射对齐救不回来 → AbsRel 虚高。三处叠加导致：

**① 主损失监督的是「视差」** — `loss/videoloss.py` `VideoDepthLoss.forward`
```python
target_inverse[valid_mask] = 1 / target[valid_mask]          # 视差 = 1/depth
loss_dict['spatial_loss'], ... = self.spatial_loss(
        prediction=prediction..., target=target_inverse...)   # 主损失对齐到视差
```
...
> 量化验证（smoke）：同一批预测，旧辅助损失（米制深度）= **29.55**，视差空间 = **0.026**，量级差约 1000 倍。weight=1.0 时它直接主导训练，把输出拽向米制深度。这与之前 perlayer 的 `deep_sup` 量纲问题是同一类。
...
**新增视差空间的辅助损失** — `train.py` `compute_aux_depth_loss_disp`
```python
gt_disp[valid] = 1.0 / gt[valid].clamp(min=1e-3)                 # 改为监督 GT 视差
...
with torch.no_grad():
    scale, shift = compute_scale_and_shift(d..., gt_s..., m_s...) # 每样本 SSI 对齐(detach)
d_aligned = scale.view(-1,1,1,1) * d + shift.view(-1,1,1,1)
diff = (d_aligned - gt_s).abs() * m_s                            # 视差空间 masked L1
```
...
正确性验证（smoke）：GT 视差的仿射变换喂入新损失 → **loss = 0.0**，证明 SSI 对齐只监督形状、正确工作。

---

## 五、需要师兄确认

1. **修复方向是否认可**：把辅助损失从「米制深度绝对 L1」改为「视差空间 + SSI 对齐」，与主损失同空间。
2. **是否本就打算配一套「深度空间」的评测**？如果 multiscale 有意输出米制深度，那就不是 bug，而是缺一套对应的 eval——这点只有你知道。
3. 若认可方向，`multiscale_fix` 重训完我会补上 AbsRel/δ1，直接和 baseline 0.0712 对比，判断多尺度 refine 的真实收益。
