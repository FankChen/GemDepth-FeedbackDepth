...
多尺度 refine head 训练本身正常收敛（loss 56.4→4.85）。但首次评测 AbsRel 0.243，异常偏高。定位后确认**不是方法差，是训练时辅助损失把模型输出从「视差空间」拉到了「米制深度空间」，与评测假设错配**。已按"复制一份、不动原件"做修复副本并重训，**AbsRel 0.243→0.0725、δ1 0.523→0.950，追平 baseline（0.0712），根因与修复均验证成立**。
