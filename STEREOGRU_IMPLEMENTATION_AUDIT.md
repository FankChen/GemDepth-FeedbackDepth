...
配对重复与汇总已作为[新独立入口](scripts/stereogru_matched_repeats.py)实现，规格见[匹配计划第 9–11 节](STEREOGRU_MATCHED_CONTROL_PLAN.md)。它不修改原训练器、不写旧 seed0 artifact。用户现已完成 seed1/2 并返回三种子汇总，最终结论见第 14 节；仍只描述一个开发场景的 seed 变异，不声称统计显著性或零样本 SOTA。

## 14. 三种子核验结束：进入修正后 GRU 的实现边界，而非继续筛 seed

记录：[20260910_matched30_three_seeds.json](results/stereogru/20260910_matched30_three_seeds.json)。原 seed0 使用 GPU4，追加 seeds1/2 在 GPU4 忙时复用已准备好的实验、改用 GPU5；同为 H20-3e，每对内使用同一设备和匹配数据/初值/预算。全部六个 run 完成、最终汇总返回 `FINAL_STEP=1000; seeds=0,1,2`，无 GRU 或后续自动训练。

开发 Scene18 上，跨 seed 的 C0→C1 均值为：

- **AbsRel `.256214→.233526`，相对均值改善 8.86%。** 配对 2/3 好，seed0 仍更差；配对差 `−.022688±.034289`。
- **RMSE `9.691719→9.400443`，相对均值改善 3.01%。** 配对 2/3 好，seed1 更差；配对差 `−.291275±.692704`。
- **δ1 `.626705→.636903`，增加 1.02 个百分点。** 三组为正，但 seed1 只增加约 .0264 个百分点，不能放大成强一致信号。
- **index-L1 `.041804→.043037`，三组均更差，均值恶化 2.95%。** 该不利证据保留；metric depth 标准指标与 absolute inverse-depth/index 目标不单调对应，不单凭聚合统计定位误差来源。
