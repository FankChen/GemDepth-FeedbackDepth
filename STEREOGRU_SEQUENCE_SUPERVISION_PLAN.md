...
## 当前执行决定：验收 v2，复用已审计 B0 seed0，推进正式 GRU

本节覆盖下文 v1 的“跨训练末态相等”验收要求，其余训练协议保持不变。

- 真实 GPU 审计 `sequence_audit_XR5svFMg/report` 的固定权重回放全部通过。
	两 clip 的前向差异约 `2.09e-7–2.38e-7`、梯度差异约 `7.68e-9–1.19e-8` 均在原容差内；
	更新后参数差异约 `2.09e-5–5.92e-5` 超过原检查，**旧旧重复和新旧比较都发生**。
	Adam 对小梯度差异的敏感性可解释这种放大机制，但不声称锁定具体 CUDA 算子或历史完整轨迹根因。
- **明确的事后验收修订**，不是称旧失败“已通过”：原失败目录和失败证书状态全部保留。
	v2 单独记录 revision digest、原完整 source/config/init/cache/order、审计文件指纹、完整 1000 步证据，
	在新的根目录只引用唯一已经跑过的 B0 seed0。无候选择优、无训练权重迁入方法初值。
	修订发生在仅见 baseline、未见 F1/S1 结果时；后续报告须披露此事。
- 不再要求**两次独立训练末态相等**。保留初值、梯度、支持域和更新记录检查；
	新训练完成后必须重新从磁盘加载其**自身**保存权重，逐轮/逐 clip/场景/总体回放，
	仍用 `rtol=1e-4, atol=1e-6`。相同权重回放失败仍阻塞下一阶段。
	审计的 updated-state=false 必须原样保留；新层不会把它篡改为通过或扩大其容差。
- 独立控制层：[experiments/sequence_acceptance_v2.py](experiments/sequence_acceptance_v2.py)。
	全部优化调用冻结的 `optimisation()`，模型/loss/数据/学习率/AdamW eps/seed/步数/门控均不变；
	原五棵源码树及 diagnostics 均不编辑、不 monkeypatch，不再要求用户重跑诊断。
- **执行顺序**：引用 B0 seed0 → 正式 B0 seed1、2 → F1 三 seed 门控 → F1 三 seed 正式 →
	S1 三 seed 门控 → S1 三 seed 正式 → 完整末步配对汇总。方法门控仍 200 步、两个训练 clip、
	两项 loss 比率 <=0.5，通过后三 seed 正式均从公共 INITIAL 重置，空优化器。
	总正式 9000 更新中已跑 1000、新增 **8000**；另有原定 1200 个丢弃门控更新，
	先前审计的六个临时更新另算，不冒充正式训练。
- 一条入口：[experiments/start_sequence_v2.sh](experiments/start_sequence_v2.sh)。
	`git show` 固定发布提交后执行；自行提取干净源码快照、后台启动、实时日志、锁定防重复。
	新输出默认 `sequence_v2_wcIm3t5e`，旧目录不写。只进入从未开始的阶段；失败/半途目录不自动重试。
	日志区分 `PREPARING_ACCEPTANCE_V2`（未更新）、`LAUNCHING_TRAINING`（启动阶段）、
	`TRAIN ... step`（真实更新）、`V2_COMPLETED`（自身权重验收通过）、`V2_STOPPED`（停止）。
- 本地完整合成 B0复用→余下基线→全部门控/方法/汇总及诊断/head/CLI：
	**41 passed / 158.58 s**；小 fixture 正式4步/门控60步，不冒充 GPU 1000/200 的实测。
	旧文件字节不变、前置拒绝、初值重置、篡改审计/replay拒绝均覆盖。
- 审计哈希是 prepare 当时观察值，不是独立签名证据；一致重写全部报告不在威胁模型内。
	**真实 v2 训练尚待用户在阿里云运行；不得声称已经起训或保证效果提升。**
